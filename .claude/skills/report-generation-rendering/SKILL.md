...
backend/agents/data_foundation_agent.py
backend/schemas.py
...
```
1. Tóm tắt đầu tư (Executive Summary)
2. Tổng quan doanh nghiệp (Company Overview)
3. Bối cảnh ngành và thị trường (Industry and Market Context)
4. Phân tích tài chính (Financial Performance) — from canonical facts only
5. Định giá (Valuation) — from valuation artifacts only
6. Luận điểm đầu tư (Investment Thesis)
7. Rủi ro chính (Key Risks)
8. Kết luận (Conclusion)
9. Phụ lục (Appendix)
   9a. Giả định (Assumptions)
   9b. Bảng định giá (Valuation tables)
   9c. Bảng nguồn trích dẫn (Evidence / citation table)
   9d. Tóm tắt đánh giá chất lượng (Evaluation summary)
```
...
| **Uncertainty must be explicit** | Use `"ước tính"`, `"dự phóng"`, `"có thể"` for forward-looking claims. Never state investment returns as guaranteed. |
