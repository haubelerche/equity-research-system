...
export const EVAL_LAYERS: EvalLayer[] = [
  ...
  },
  {
    ...
    metrics: [
      ...
      metric("judge_calibration_agreement", "Mức đồng thuận calibration của judge", "Judge calibration agreement", "", "gte", 0.85, "Judge Calibration Set", "Mức đồng thuận của LLM Judge với deterministic labels, seeded defects và human review samples", { aliases: ["agent.judge_calibration_agreement"], thresholdLabel: ">= 85%", metricType: "score", scope: "benchmark_suite", severity: "P1", blocksPublish: false }),
      metric("judge_rationale_evidence_coverage", "Độ bao phủ evidence trong rationale của judge", "Judge rationale evidence coverage", "%", "gte", 0.9, "Judge Output Audit", "Số fail/warning của judge có trace span, artifact hoặc rubric clause / Tổng fail/warning", { aliases: ["agent.judge_rationale_evidence_coverage"], thresholdLabel: ">= 90%", metricType: "coverage", scope: "benchmark_suite", severity: "P2", blocksPublish: false }),
    ],
    methodology: [
  ...
  },
  {
    ...
    metrics: [
      ...
      metric("report.thesis_specificity", "Độ cụ thể của investment thesis", "Thesis specificity", "", "gte", 80, "Report Quality Rubric", "Điểm rubric cho thesis riêng theo công ty, driver trọng yếu và điều kiện bác bỏ", { thresholdLabel: ">= 80%", metricType: "score", scope: "report_run", severity: "P2", blocksPublish: false }),
      metric("report.risk_catalyst_quality", "Chất lượng rủi ro và catalyst", "Risk and catalyst quality", "", "gte", 80, "Report Quality Rubric", "Điểm rubric cho xác suất, thời điểm, đường truyền tài chính và evidence của risk/catalyst", { thresholdLabel: ">= 80%", metricType: "score", scope: "report_run", severity: "P2", blocksPublish: false }),
      metric("report.recommendation_consistency", "Tính nhất quán của khuyến nghị", "Recommendation consistency", "%", "gte", 100, "Recommendation Consistency Gate", "Tỷ lệ nhất quán giữa rating, target price, market price, upside/downside và risk language", { thresholdLabel: "= 100%", metricType: "coverage", scope: "report_run", severity: "P0", blocksPublish: true }),
      metric("report.sensitivity_disclosure_completeness", "Độ đầy đủ sensitivity disclosure", "Sensitivity disclosure completeness", "%", "gte", 90, "Report Quality Rubric", "Tỷ lệ driver trọng yếu có sensitivity hoặc scenario disclosure phù hợp", { thresholdLabel: ">= 90%", metricType: "coverage", scope: "report_run", severity: "P1", blocksPublish: false }),
      metric("report.peer_industry_context_quality", "Chất lượng bối cảnh peer và ngành", "Peer and industry context quality", "", "gte", 75, "Report Quality Rubric", "Điểm so sánh peer/ngành về growth, margin, valuation và balance sheet", { thresholdLabel: ">= 75%", metricType: "score", scope: "report_run", severity: "P2", blocksPublish: false }),
      metric("report.executive_summary_actionability", "Tính hành động của executive summary", "Executive summary actionability", "", "gte", 80, "Report Quality Rubric", "Điểm summary có recommendation, valuation basis, driver, risk và monitoring trigger", { thresholdLabel: ">= 80%", metricType: "score", scope: "report_run", severity: "P2", blocksPublish: false }),
    ],
    methodology: [
