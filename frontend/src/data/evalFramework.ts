...
export const EVAL_LAYERS: EvalLayer[] = [
  {
    ...
    metrics: [
      ...
      metric("valuation_method_data_readiness", "Mức sẵn sàng dữ liệu cho phương pháp định giá", "Valuation method data readiness", "", "gte", 1, "Valuation Publishability Policy", "Final report publishable và recommendation publishable theo policy định giá", { thresholdLabel: "= true", metricType: "boolean", scope: "report_run", severity: "P0", blocksPublish: true }),
  ...
  },
  {
    ...
    metrics: [
      ...
      metric("valuation_regression_failures", "Số kết quả định giá chuẩn sai lệch vượt ngưỡng", "Golden valuation regression failures", "", "lte", 0, "Golden Valuation Regression", "Số case trong golden valuation regression fail", { aliases: ["golden_drift_out_of_tolerance"], thresholdLabel: "= 0", metricType: "error_count", scope: "benchmark_suite", severity: "P0", blocksPublish: true }),
      metric("share_count_mismatch", "Số sai lệch cổ phiếu dùng trong EPS hoặc giá mục tiêu", "Share count mismatch", "", "lte", 0, "Share Count Reconciliation", "Số case share count mismatch", { thresholdLabel: "= 0", metricType: "error_count", scope: "report_run", severity: "P0", blocksPublish: true }),
      ...
      metric("unexplained_forecast_anomalies", "Số bất thường dự phóng trọng yếu thiếu giải trình", "Unexplained forecast anomaly", "", "lte", 0, "Forecast Quality Gate", "Số bất thường trọng yếu không có explanation hoặc catalyst", { aliases: ["forecast_anomaly_unexplained"], thresholdLabel: "= 0", metricType: "error_count", scope: "report_run", severity: "P1", blocksPublish: true }),
      metric("recommendation_inconsistency", "Số lỗi khuyến nghị không nhất quán với định giá", "Recommendation inconsistency", "", "lte", 0, "Recommendation Policy Gate", "Số case rating không khớp target price, current price và upside/downside", { thresholdLabel: "= 0", metricType: "error_count", scope: "report_run", severity: "P0", blocksPublish: true }),
    ],
    methodology: [
