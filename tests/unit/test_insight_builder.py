"""Regression tests for deterministic, materiality-gated report insights."""
...
def test_growth_quality_flags_cost_driven_when_profit_outpaces_revenue() -> None:
    ...
    assert growth["valuation_implication"]
    assert "biên" in growth["claim"].lower() or "chi phí" in growth["claim"].lower()


def test_non_material_growth_spread_is_not_ready() -> None:
    pack = build_insight_pack(_inputs(revenue_growth_latest=0.04, profit_growth_latest=0.06))
    growth = next(i for i in pack if i["section"] == "growth")
    assert growth["status"] == "insufficient_evidence"
    assert "material_growth_spread" in growth["missing_fields"]
...
def test_net_cash_leverage_insight() -> None:
    ...
    assert "tiền" in lev["claim"].lower()


def test_middle_leverage_does_not_emit_generic_ready_insight() -> None:
    pack = build_insight_pack(_inputs(net_debt_ebitda=0.8))
    lev = next(i for i in pack if i["section"] == "leverage")
    assert lev["status"] == "insufficient_evidence"
    assert "material_leverage_signal" in lev["missing_fields"]
