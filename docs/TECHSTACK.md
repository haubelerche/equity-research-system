...
| Multi-agent harness | `backend/harness/`, `config/agents/agents.yml`, `config/agents/prompts/`, `config/harness/` | Xác định graph, agent roles, model adapter, tool registry, gates, và typed contracts |
...
Tài liệu tech stack trước đây chưa đủ cho một nhóm mới tái dựng môi trường sạch vì nó trộn lẫn ba lớp khác nhau: công nghệ đang được runtime sử dụng, dependency chỉ tồn tại trong manifest, và dependency được import trong code nhưng chưa được khai báo. Với một hệ thống AI tài chính, sự lệch pha này không chỉ là vấn đề tài liệu; nó ảnh hưởng trực tiếp đến khả năng triển khai Docker, reproducibility của report, kiểm soát hallucination, và cost-to-serve của các agent.
...
| Dependency manifest | `requirements.txt` | Có manifest tối thiểu nhưng chưa đầy đủ so với import thực tế | `requirements.txt`, quét import trong `backend/`, `scripts/`, `tests/` |
| Package manager | `pip` | Docker cài bằng `pip install -r requirements.txt`; chưa có lockfile | `Dockerfile` |
...
| Agent registry | YAML + Pydantic | Khai báo 6 agent, prompt path, model, timeout, allowed tools | `config/agents/agents.yml`, `backend/harness/agent_registry.py` |
| Agent roles | ResearchManager, DataEvidence, FinancialAnalysis, ForecastValuation, ThesisReport, SeniorCritic | Phân tách responsibility để giảm context bottleneck và tăng auditability | `config/agents/agents.yml` |
...
| Migration model | SQL migrations + custom runner | Migrations `001` đến `034`, ghi `public.schema_migrations` | `backend/database/migrate.py`, `backend/database/migrations/` |
| Runtime schema floor | `030_supabase_storage_contract` | RuntimeStore kiểm tra schema tối thiểu cho storage contract | `backend/runtime_store.py` |
| Highest migration detected | `034_runs_status_add_blocked.sql` | Trạng thái migration cao nhất trong repo | `backend/database/migrations/` |
...
Gap manifest: `vnstock`, `requests`, và `beautifulsoup4` đang được import nhưng chưa được khai báo trực tiếp trong `requirements.txt`. Đây là thiếu hụt nghiêm trọng cho môi trường sạch vì các crawler và connector tài chính không thể giả định dependency bắc cầu.
...
Gap manifest và system dependency: `pdfkit` và `pypdf` đang được import nhưng chưa có trong `requirements.txt`; `wkhtmltopdf` và Chrome/Edge không được cài trong Dockerfile; font Unicode cho PDF fallback phụ thuộc `assets/fonts/` hoặc system fonts. WeasyPrint có trong requirements nhưng trên một số nền tảng cần thêm native libraries/fonts, vì vậy PDF export cần được kiểm chứng trong image thật thay vì chỉ kiểm tra import.
...
| Make targets | `make test`, `make audit` | Lệnh chạy test canonical nhưng cần kiểm tra lại file path trong `audit` target | `Makefile` |

Rủi ro hiện tại: `Makefile` có `audit` target trỏ đến một số file test như `tests/unit/test_six_agent_workflow.py` và `tests/unit/test_tool_registry.py`; cần xác nhận các file này còn tồn tại trước khi xem `make audit` là CI canonical. Không phát hiện `.github/workflows` hoặc cấu hình CI trong danh sách file, nên automation kiểm thử có thể chưa được chuẩn hóa ở repo này.
...
### 1. Sửa dependency manifest trước khi mở rộng tính năng
...
| CI absent | Thêm workflow chạy import smoke, unit tests, storage layout tests, và một target kiểm tra `requirements.txt` khớp import thực tế |
