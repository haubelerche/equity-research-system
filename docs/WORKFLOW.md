...
| 8 | `EXPORT_GATES` | Ghi evidence packet, chạy FPTS grade và package validation | `fpts_grade_evaluation`, `quality_gate`, `publishable_final_report_model` |
| 9 | `PUBLISH` | Xác nhận publishable model và ghi manifest | `manifest.json`, trạng thái `auto_exported` |
...
Gate có `severity="critical"` sẽ đặt run thành `blocked`. Gate severity warning vẫn ghi issue nhưng không nhất thiết dừng run, tùy aggregate gate. Các gate quan trọng gồm `DATA_QUALITY_GATE`, `FORECAST_QUALITY_GATE`, `VALUATION_GATE`, `VALUATION_RECONCILIATION_GATE`, `REPORT_ASSEMBLY_GATE`, `REPORT_COMPLETENESS_GATE`, `SENIOR_CRITIC_GATE`, `CITATION_GATE`, `FPTS_GRADE_GATE`, `PACKAGE_VALIDATION_GATE` và `EXPORT_GATE`.
