...
def test_ragas_samples_have_ground_truth_and_context_policy():
    ...
    assert all(r.get("question") for r in answerable)
    assert all(r.get("answer") or r.get("ground_truth") for r in answerable)
    assert all(r["contexts"] for r in answerable)
    assert all(r.get("metadata", {}).get("ticker") for r in answerable)
    assert all(r.get("metadata", {}).get("fiscal_year") for r in answerable)
