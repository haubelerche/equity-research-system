...
CONNECTOR_VERSION = "vnstock_company_connector_v1"
...
def _register_payload(
    ...
    return registry.register_version(
        SourceVersionInput(
            source_id="company_news" if endpoint in {"news", "events"} else "listing_metadata",
            source_uri=source_uri,
            source_type="catalyst_company_news" if endpoint in {"news", "events"} else "market_reference",
            checksum=checksum,
...
def sync_company_ticker(ticker: str, segment: str, store: PostgresFactStore, registry: SourceRegistry) -> dict[str, int]:
    client = Vnstock(symbol=ticker, source="KBS")
    overview = _to_records(client.company.overview())
    shareholders = _to_records(client.company.shareholders())
    officers = _to_records(client.company.officers())
    news = _to_records(client.company.news())
    events = _to_records(client.company.events())
    ...
    store.upsert_company_profile(
        ticker=ticker,
        company_name=first_overview.get("companyName") or first_overview.get("company_name"),
    ...
    catalyst_rows = _build_events(ticker=ticker, rows=news, source_version_id=news_version, event_type="company_announcement")
    catalyst_rows.extend(_build_events(ticker=ticker, rows=events, source_version_id=events_version, event_type="company_announcement"))
