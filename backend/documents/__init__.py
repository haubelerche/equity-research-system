...
def _slug_label(text: str) -> str:
    ...
    # Drop any remaining non-ASCII characters (e.g. Ä‘ â†’ d already handled by NFD
    # but Ä‘ is a special case: NFD of Ä‘ is still Ä‘ because it's a base letter)
    # Handle Ä‘/Ä explicitly:
    ascii_only = ascii_only.replace("Ä‘", "d").replace("Ä", "D")
...
def _parse_eps_raw(raw: str) -> Optional[float]:
    """Parse EPS value (VND/share) â€” no scaling, just clean and convert."""
    if not raw or raw.strip() in ("â€”", "-", "", "n/a", "N/A"):
        return None
...
def _parse_vnd_bn(raw: str) -> Optional[float]:
    """Parse a raw Vietnamese financial string to tá»· VND (billions VND).

    Returns None for empty/null markers.
    Handles:
    - Negative values in parentheses: "(273,600)" â†’ -273.6
    - Comma-separated numbers: "4,127,400"
    - Values > 500,000 assumed to be in triá»‡u VND â†’ divide by 1_000 to get tá»·
    - Values â‰¤ 500,000 assumed already in tá»· VND
    """
    stripped = raw.strip()
    ...
    # Commas indicate the value is expressed in triá»‡u VND and needs /1000 scaling.
    ...
    # Scale: values expressed with comma-thousands-separators are in triá»‡u VND.
    # Values > 500,000 (absolute) are also assumed to be in triá»‡u VND even without
...
# ---------------------------------------------------------------------------
# Label â†’ metric mapping (loaded from config/financial_metric_dictionary.yaml)
# ---------------------------------------------------------------------------
...
class PdfType(str, Enum):
    ...
    """PDF has a text layer â€” pdfplumber can extract tables directly."""
    SCANNED = "scanned"
    """PDF is a scanned image â€” needs OCR before table extraction."""
    UNKNOWN = "unknown"
...
def extract_from_pdf_ocr(
    ...
    try:
        ...
        print(
            f"[pdf_extractor] OCR unavailable â€” install {missing}: "
