...
Jobs:
  weekly_sync     â€” run ingest_ticker + build_facts for all active MVP tickers
  daily_prices    â€” refresh market price data (catalyst / price updates)
  monthly_valuation â€” recompute valuation artifacts for all active tickers
...
def job_daily_prices(ticker: str | None = None) -> dict:
    ...
    Runs Mondayâ€“Friday at 18:00 ICT (11:00 UTC) after market close.
    """
    tickers = [ticker.strip().upper()] if ticker else MVP_TICKERS
    results: dict[str, str] = {}
    for t in tickers:
...
_JOB_REGISTRY: dict[str, dict] = {
    ...
    "daily_prices": {
        "fn": job_daily_prices,
        "description": "Refresh market prices (Monâ€“Fri at 11:00 UTC)",
...
def start() -> None:
    ...
    if not _APScheduler_available:
        logger.warning("[scheduler] APScheduler not installed â€” scheduler disabled. pip install apscheduler")
        return
