"""Deterministic FPTS-grade report discipline gates and scoring."""
...
def _has_bridge(payload: dict[str, Any], *names: str) -> bool:
...
def forecast_reasonableness_gate(forecast: dict[str, Any]) -> dict[str, Any]:
    ...
    forecast = _normalized_forecast(forecast)
    rows = _rows(forecast)
    has_profit_bridge = _has_bridge(forecast, "profit_bridge", "margin_bridge")
    has_operating_bridge = _has_bridge(
        forecast, "operating_leverage_bridge", "margin_bridge",
    )
    ...
    for previous, current in zip(rows, rows[1:]):
        ...
        if revenue_growth is not None and revenue_growth >= 0:
            ...
            if profit_growth is not None and profit_growth > threshold and not has_profit_bridge:
                reasons.append(f"profit_growth_requires_bridge:{label}")
            if eps_growth is not None and eps_growth > threshold and not has_profit_bridge:
                reasons.append(f"eps_growth_requires_bridge:{label}")

        ebit_margin_change = None
        if isinstance(previous.get("ebit_margin"), (int, float)) and isinstance(current.get("ebit_margin"), (int, float)):
            ebit_margin_change = float(current["ebit_margin"]) - float(previous["ebit_margin"])
        gross_margin_change = None
        if isinstance(previous.get("gross_margin"), (int, float)) and isinstance(current.get("gross_margin"), (int, float)):
            gross_margin_change = float(current["gross_margin"]) - float(previous["gross_margin"])
        if (
            ebit_margin_change is not None
            and ebit_margin_change > 0.03
            and (gross_margin_change is None or gross_margin_change < ebit_margin_change - 0.01)
            and not has_operating_bridge
        ):
            reasons.append(f"ebit_margin_jump_without_gross_margin_support:{label}")
    ...
    product_count = len(revenue.get("by_product_group") or {})
    if channel_count < 2:
        reasons.append("pharma_driver_channels_insufficient")
    if product_count < 1:
        reasons.append("pharma_driver_product_groups_missing")
...
def company_research_depth_gate(company_research_pack: dict[str, Any]) -> dict[str, Any]:
    ...
    missing = [field for field in required if not company_research_pack.get(field)]
...
def fpts_grade_gate(state: dict[str, Any]) -> dict[str, Any]:
    ...
    if result["passed"]:
        return pass_gate("FPTS_GRADE_GATE", result)
    reasons = [
        reason
