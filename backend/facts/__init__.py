"""Generate a Data Validation Report in markdown format (Plan Â§19).
...
def generate_data_validation_report(
    ...
    Args:
        ...
        fact_table: FactTable dict (taxonomy_key â†’ period â†’ value)
    ...
    for period in periods_available:
        # Mark âœ“ for collected periods
        coverage_rows.append(f"| {period} | âœ“ | pass |")

    periods_missing = fy_report.get("periods_missing", [])
    for period in periods_missing:
        # Mark âœ— for missing periods
        coverage_rows.append(f"| {period} | âœ— | fail |")

    coverage_table = "\n".join(coverage_rows) if coverage_rows else "| (no periods) | | |"

    coverage_gate_status = fy_report.get("coverage_gate", "unknown")
    coverage_gate_symbol = "âœ“" if coverage_gate_status == "pass" else "âœ—"
    ...
    overall_valuation_gate = "âœ“ PASS" if valuation_ready else "âœ— FAIL"
    ...
    if blocking_reasons:
        ...
        blocking_reasons_str = "None â€” valuation may proceed"
    ...
    report_md = f"""# Data Validation Report â€” {ticker}
