"""Ticker mapping for catalyst events â€” Source-Provenance Rebuild, Phase 5.

Maps an event to a specific ticker (explicit) or to a sector when no single ticker can
be attributed (sector_level). An event must be one or the other â€” never unmapped.
"""
...
_NAME_HINTS: dict[str, str] = {
    "dÆ°á»£c háº­u giang": "DHG", "dhg pharma": "DHG", "dhg": "DHG",
...
def map_event_to_ticker(text: str, explicit_ticker: str | None = None) -> TickerMapping:
    ...
    If an explicit ticker is given (and valid) â†’ explicit. Else scan text for a company
    name/ticker hint. If none found â†’ sector_level (never silently unmapped).
    """
