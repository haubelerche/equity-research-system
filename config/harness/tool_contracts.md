...
| `read_snapshot` | ticker, explicit snapshot id | metric ids, FY periods, source tiers, evidence refs | read-only | Blocks `FINANCIAL_ANALYST_RUN` when snapshot id or snapshot facts are unavailable. |
| `read_ratio_artifact` | ticker, explicit snapshot id | ratio artifact path, metric ids, FY periods | read-only | Blocks `FINANCIAL_ANALYST_RUN` when ratio artifact cannot be derived from the snapshot. |
...
| `generate_report` | ticker, snapshot id, FY range, mode | report path, citation map path, claim counts, source-tier gate | read-write artifact | Blocking for final export when material claims lack traceable citations. |
