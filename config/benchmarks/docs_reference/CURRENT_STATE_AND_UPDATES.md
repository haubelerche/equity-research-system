...
| Danh mục báo cáo | `/reports` hiển thị active configured universe, ưu tiên run manifest/artifact lineage và fallback local preview khi thiếu object production. | `backend/api.py`, `backend/reporting/output_inventory.py` |
...
```mermaid
flowchart TD
    A[/reports] --> B[Load active configured universe từ frontend universe]
    B --> C[GET /reports]
...
| P0 | Dùng MVP5 làm phạm vi định lượng chính, dùng active configured universe làm readiness matrix | Tránh làm loãng evidence bằng các ticker chưa cần phân tích sâu ở mức thesis |
